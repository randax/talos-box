...
require 'sudo test -s "$state_file"' "$systemd_harness"
require 'already exists and is non-empty' "$systemd_harness"
...
require 'sudo mv "$state_backup" "$state_file" 2>/dev/null || true' "$systemd_harness"
