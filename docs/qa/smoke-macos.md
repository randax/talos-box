...
2. `git -C <talos-box checkout> rev-parse HEAD` — record the commit if running from source.
   - **Record the host cache state too**, because C1's timing bar depends on it: `tbx cache list` (does a Talos disk image for the pinned version already exist?) and the `image-cache` / `mirror-health` lines from step 3. Report it as `cold` (no matching disk image cached) or `warm` (image already cached) — a warm host skips the image download entirely and can finish C1 in seconds.
