...
- `/var/lib/tbx/reservations.json` does not exist before the run. Any existing file — even an empty one — means the host already carries helper state; preserve that machine or restore/delete the file before using this disposable-host harness.
...
6. The stronger #514 sequence stops the helper, moves `reservations.json` aside, starts from empty persisted state, and polls for up to about 90 seconds for periodic `net.sync` to restore the file and both DHCP listeners. The moved-aside copy is test-created state (preflight guarantees no pre-run file exists): it is restored only by the failure cleanup trap, and deleted once recovery succeeds so a later exit cannot resurrect reservations for destroyed clusters.
