...
| `balloon` | INFO only: the daemon was started with `TBX_DISABLE_BALLOON` and launches guests without a memory balloon device | Never a WARN or FAIL; the line is absent when ballooning is on. The flag does **not** prevent the `Kernel tag check fault` host panics it was once suggested for — see [macOS kernel panics](macos-panics.md) |
...
| `security-inventory` | Lists activated system extensions — VPN, EDR, and content-filter software known to interfere with guest TLS, DNS, or routes | Mostly informational; use it to explain mirror, DNS, or route findings above. `WARN`s when a content-filter extension is active on a memory-tagging (M5-generation) host: a macOS kernel bug can then panic the whole Mac when filtered TCP sockets close — see [macOS kernel panics](macos-panics.md) (#513) |
