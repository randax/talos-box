...
| `host-pressure` | Host memory pressure, swap exhaustion, and free space on the volume holding `~/.talosbox` are outside the range that resets guests and corrupts Talos `EPHEMERAL` data | `FAIL`s where `tbx cluster create` would refuse and `WARN`s where it would only warn, printing the same runnable remediation — free host memory (`tbx down`) or host storage (`tbx cache prune --all`) — and noting that `--force` overrides the create gate at the risk of guest-disk corruption. macOS can retain swap after pressure clears, so high stale swap is only a warning when measured free RAM covers the balloon reserve plus the guests about to start; critical pressure, insufficient RAM, or an unmeasurable free-memory reading still blocks. A `PASS` prints the three numbers it was decided on — free memory, swap in use, free space on the `~/.talosbox` volume — and how much room is left for new guests once the balloon reserve is kept free. That number applies only **while other guests are already running** — the provision-start gate does not fire at all on an idle host, where a create of any size is admitted without a headroom check — and it is not a hard admission line even then: before refusing, the gate credits what the balloon controller can take back from the running guests, so a create is blocked only when the shortfall exceeds that credit |
